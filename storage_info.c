...
// Helper struct untuk menyimpan mapping disk fisik ke logis
typedef struct
{
    char logicalDrive[4];
...
// Helper function untuk mendapatkan informasi disk fisik dan mappingnya
...
// Fungsi helper untuk mendapatkan semua logical disks untuk sebuah physical disk
        ...
        while (SUCCEEDED(pPartEnum->lpVtbl->Next(pPartEnum, WBEM_INFINITE, 1, &pPartObj, &uPartReturn)) && uPartReturn != 0)
        {
            ...
            if (SUCCEEDED(pPartObj->lpVtbl->Get(pPartObj, L"DeviceID", 0, &vtPartID, 0, 0)))
            {
                ...
                if (executeWQLQuery(session, partQuery, &pLogicalEnum))
                {
                    ...
                    // Enumerate all logical disks for this partition
                    while (SUCCEEDED(pLogicalEnum->lpVtbl->Next(pLogicalEnum, WBEM_INFINITE, 1, &pLogicalObj, &uLogicalReturn)) && uLogicalReturn != 0)
                    {
                        // Get drive letter
                        ...
                        // Get volume name untuk model jika kosong
